...
public interface MessageMapper extends JpaRepository<Message, Long> {
    ...
    @Query(value = "SELECT m.*, u.name as sender_name, u.avatar as sender_avatar " +
            ...
            "WHERE m.type = 'chat' AND ((m.user_id = ?1 AND m.sender_id = ?2) OR (m.user_id = ?2 AND m.sender_id = ?1)) "
            +
            "ORDER BY m.created_at ASC", nativeQuery = true)
    List<Map<String, Object>> getConversation(Long userId, Long contactId);
}
