...
public class ErrorController {
    ...
    public Result<ErrorItem> getErrorById(@PathVariable String id) {
    ...
    public Result<ErrorItem> addError(@RequestBody ErrorItem errorItem) {
        if (errorItem.getQuestion() == null || errorItem.getQuestion().trim().isEmpty()) {
            return Result.fail(40001, "题目不能为空");
        }
        if (errorItem.getCreatedAt() == null) {
            errorItem.setCreatedAt(System.currentTimeMillis());
        }
    ...
    public Result<Map<String, Object>> deleteError(@PathVariable String id) {
        Optional<ErrorItem> errorItem = errorRepository.findById(id);
        if (errorItem.isEmpty()) {
            return Result.fail(40401, "错题不存在");
        }
    ...
    public Result<ErrorItem> updateError(@PathVariable String id, @RequestBody ErrorItem errorItemRequest) {
    ...
    public Result<Map<String, Object>> analyzeError(@RequestBody Map<String, Object> request) {
        ...
        if (request.containsKey("errorId")) {
            errorId = request.get("errorId") != null ? request.get("errorId").toString() : null;
        }
