...
public class LearningPlanController {
    ...
    public Result<Map<String, Object>> checkAIStatus() {
        Map<String, Object> status = new HashMap<>();
        
        try {
            boolean available = aiService.checkAvailability();
            status.put("available", available);
            status.put("message", available ? "AI服务可用" : "AI服务暂不可用，将使用本地算法");
            status.put("timestamp", OffsetDateTime.now().toString());
        } catch (Exception e) {
            log.warn("AI服务检查失败：{}", e.getMessage());
            status.put("available", false);
            status.put("message", "AI服务检查失败：" + e.getMessage());
            status.put("timestamp", OffsetDateTime.now().toString());
        }
